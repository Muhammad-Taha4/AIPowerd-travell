...
try:
    ...
    with conn.cursor() as cursor:
        cursor.execute("SELECT * FROM users WHERE email='test2@fusix.com'")
        user = cursor.fetchone()
        if user:
            print(f"User found in MySQL DB: {user}")
            
            # Check profile too
            cursor.execute("SELECT * FROM traveller_profiles")
            profiles = cursor.fetchall()
            print(f"Total profiles in MySQL: {len(profiles)}")
        else:
